...
public class AtualizarSenhaUseCaseIT {
    ...
    void deveAtualizarSenhaComSucesso() {
        ...
        String senhaAntes = usuarioAntesDeAtualizar.getLogin().getSenha();

        // Act
        atualizarSenhaUseCase.atualizar(usuarioId, novaSenha);

        var usuarioAposAtualizar = usuarioGateway.buscarPorId(usuarioId);
        String senhaDepois = usuarioAposAtualizar.getLogin().getSenha();

        // Assert
        assertThat(senhaDepois).isNotEqualTo(senhaAntes);
        assertThat(senhaDepois).isEqualTo(novaSenha);
    }
